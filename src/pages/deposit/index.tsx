import { Box, Stack, TableContainer, Table, Thead, Tr, Th, Tbody, Td, Text, Spacer, Flex, Link, Button, Input, FormLabel } from '@chakra-ui/react';
...
const Deposit = () => {
    ...
    const getDepositByLeafIdx = (id: string) => {
        const depositsJSONLS = getLocalStorageValue('deposits');
        const depositsArr = depositsJSONLS ? JSON.parse(depositsJSONLS) : [];
        const depositToWithdraw = depositsArr.filter(deposit => deposit.leaf_idx === parseInt(id))[0];
    ...
    const exportData = (id: string) => {
        ...
        const answer = window.confirm('You have exported deposit. Do you want to delete deposit from the app?');
        if(answer) {
            const depositsJSONLS = getLocalStorageValue('deposits');
            const depositsArr = depositsJSONLS ? JSON.parse(depositsJSONLS) : [];
            const depositsWithoutCurrent = depositsArr.filter(deposit => deposit.leaf_idx !== parseInt(id));
                        ...
                        {deposits.map((deposit, index) => (
                            <Tr key={index}>
                                ...
                                <Td>
                                    ...
                                    <Button size={'sm'} onClick={() => exportData(deposit.leaf_idx)}>Export</Button>
                                    </Flex>
                                </Td>
                            </Tr>
                        ))}
